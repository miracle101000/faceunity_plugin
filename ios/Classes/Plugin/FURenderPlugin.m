...
- (void)startPreviewingVideo {
    ...
    if (previewBuffer == NULL) {
        // 获取视频首帧
        UIImage *previewImage = [FUUtil previewImageFromVideoURL:self.selectedVideoURL preferredTrackTransform:NO];
...
- (void)selectWithMediaType:(NSString *)type {
    ...
    [[FUUtil topViewController] presentViewController:picker animated:YES completion:nil];
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<UIImagePickerControllerInfoKey,id> *)info {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([mediaType isEqualToString:(NSString *)kUTTypeImage]) {
            ...
            // 获取视频地址
            [FUUtil requestVideoURLFromInfo:info resultHandler:^(NSURL * _Nonnull videoURL) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (videoURL) {
                        self.selectedVideoURL = videoURL;
                        self.videoOrientation = [FUUtil videoOrientationFromVideoURL:self.selectedVideoURL];
...
- (void)videoReaderDidFinishReading {
    ...
    UIImage *lastImage = [FUUtil lastFrameImageFromVideoURL:self.selectedVideoURL preferredTrackTransform:NO];
